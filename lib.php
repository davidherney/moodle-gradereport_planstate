...
/**
 * Definition of the grade_planstate_report class
 *
...
/**
 * Class providing an API for the planstate report building and displaying.
 * @uses grade_report
 * @package gradereport_planstate
 */
class grade_report_planstate extends grade_report {
     ...
     */
    public function __construct($userid, $gpr, $context, $categoryid) {
        ...
        $this->courses = enrol_get_users_courses($this->user->id, false, 'id, shortname, showgrades, category, enablecompletion');
        ...
        if ($this->courses) {
            foreach ($this->courses as $course) {
                ...
                if ($categoryid !== null && $course->category != $categoryid) {
                    continue;
                }

                $this->showrank[$course->id] = grade_get_setting($course->id, 'report_planstate_showrank', !empty($CFG->grade_report_planstate_showrank));
                if ($this->showrank[$course->id]) {
                    $this->showrank['any'] = true;
                }

                $this->showtotalsifcontainhidden[$course->id] = grade_get_setting($course->id, 'report_planstate_showtotalsifcontainhidden', $CFG->grade_report_planstate_showtotalsifcontainhidden);
        ...
        $this->baseurl = $CFG->wwwroot.'/grade/planstate/index.php?id='.$userid;
    ...
    public function setup_table() {
        ...
        $tablecolumns[] = 'completed';
        $tableheaders[] = $this->get_lang_string('completed', 'completion');

        $this->table = new flexible_table('grade-report-planstate-'.$this->user->id);
        ...
        $this->table->set_attribute('id', 'planstate-grade');
    ...
    public function setup_courses_data($studentcoursesonly) {
        ...
        foreach ($this->courses as $course) {
            ...
            if ($course->enablecompletion) {
                $completed = $DB->get_record('course_completions', array('course' => $course->id, 'userid' => $this->user->id));

                $coursesdata[$course->id]['completed'] = $completed->timecompleted ? $completed->timecompleted : false;
            } else {
                $coursesdata[$course->id]['completed'] = null;
            }
        }
        return $coursesdata;
    }
    ...
    public function fill_table($activitylink = false, $studentcoursesonly = false) {
        ...
        if ($this->courses) {
            ...
            $timedateformat = get_string('strftimedatetimeshort');
            ...
            foreach ($coursesdata as $coursedata) {
                ...
                $data[] = $completed === null ? '-' : ($completed === false ? 'No' : userdate($completed, $timedateformat));
    ...
    public static function viewed($context, $courseid, $userid) {
        $event = \gradereport_planstate\event\grade_report_viewed::create(
            array(
                'context' => $context,
                'courseid' => $courseid,
...
function grade_report_planstate_settings_definition(&$mform) {
    global $CFG;
    ...
    if (empty($CFG->grade_report_planstate_showrank)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[1]);
    }

    $mform->addElement('select', 'report_planstate_showrank', get_string('showrank', 'grades'), $options);
    $mform->addHelpButton('report_planstate_showrank', 'showrank', 'grades');
    ...
    if (!array_key_exists($CFG->grade_report_planstate_showtotalsifcontainhidden, $options)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[$CFG->grade_report_planstate_showtotalsifcontainhidden]);
    }

    $mform->addElement('select', 'report_planstate_showtotalsifcontainhidden', get_string('hidetotalifhiddenitems', 'grades'), $options);
    $mform->addHelpButton('report_planstate_showtotalsifcontainhidden', 'hidetotalifhiddenitems', 'grades');
}
 ...
 * @param stdClass $course Course object
 */
function gradereport_planstate_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    if (empty($course)) {
